  ...
  return {
    ...
    async transformIndexHtml(html) {
      ...
      if (config.mode === 'development') {
        const auth_info_file = await fs.readFile(path.join(_dirname, '..', '.tmp', 'auth_info.json'), {
          encoding: 'utf-8',
        });
        ...
        return html.replace(regex, `<script src="http://localhost:${port}/__neutralino_globals.js"></script>`);
      }
...
export default defineConfig({
  plugins: [react(), tsconfigPaths(), neutralino()],
  server: {
    port: 3000,
    strictPort: true,
  },
